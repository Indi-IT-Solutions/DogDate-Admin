...
const ReceivedRequests: React.FC = () => {
    ...
    const fetchReceivedRequests = async (page: number = 1, limit: number = 10, search?: string, type?: string) => {
        if (!userId) return;
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            if (searchText !== undefined) {
                fetchReceivedRequests(1, pagination.perPage, searchText);
            }
        }, 500);
    ...
    const handlePageChange = (page: number) => {
        fetchReceivedRequests(page, pagination.perPage, searchText);
    };

    const handlePerRowsChange = (perPage: number, page: number) => {
        fetchReceivedRequests(page, perPage, searchText);
    };
            ...
            cell: (row: any) => (
                <div className="d-flex gap-2 align-items-center">
                    ...
                    <div>
                        ...
                        <span className="badge bg-info">Request</span>
                    </div>
                </div>
            ),
        },
    ...
    return (
        <React.Fragment>
            ...
            <DataTable
                ...
                noDataComponent={
                    <div className="text-center py-4">
                        <Icon icon="mdi:inbox" width={48} height={48} className="text-muted mb-2" />
                        <p className="text-muted">
                            {loading ? 'Loading requests...' : 'No received requests found'}
                        </p>
                        {!loading && pagination.totalRows === 0 && (
                            <small className="text-muted">This user hasn't received any match requests yet</small>
                        )}
                    </div>
                }
            />
